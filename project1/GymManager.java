...
public class GymManager {
    ...
    private boolean hasValidDate(String[] inputString){
        ...
        if(isValidDate(DOBString)){
            ...
            if(testDate.isValid()){
                if(command == Command.REMOVE || testDate.isEighteen()){
                    return true;
                }else{
                    if(!testDate.isBorn()){
     ...
     */
    private void loadController(String command){
        switch(command){
            case "LS" -> loadClasses();
            case "LM" -> loadMembers();
        }
    ...
    private void guestHelper(Command command, Member testMember, FitnessClass testClass){
        ...
        if(familyMember.getGuessPasses() > 0){
            if(validLocationSelection(familyMember, testClass)){
                ...
                testClass.addGuest(familyMember);
    ...
    public void run () {
        ...
        while(true) {
            ...
            switch (command) {
                ...
                case LOAD, LOAD_CLASS, LOAD_MEMBER -> loadController(inputArray[0]);
